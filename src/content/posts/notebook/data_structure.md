...
假设有 $n$ 个边和坐标轴平行的矩形，我们要求这些矩形覆盖区域的并集面积。
...
到 $x=x_1$ 时，这个矩形开始生效，所以 $[y_1,y_2)$ 的覆盖次数 $+1$；到 $x=x_2$ 时这个矩形失效，所以 $[y_1,y_2)$ 的覆盖次数 $-1$。

我们定义事件如下，其中 $k=1$ 表示进入矩形，$k=-1$ 表示离开矩形。
...
我们实际上只需要处理这些事件。假设所有事件横坐标为：$x_1<x_2<x_3<\cdots$，考虑  $[x_i,x_{i+1})$，在这一段里面不存在任何矩形左边界或右边界。因此哪些矩形正在覆盖扫描线，不会发生变化。所以事件之间的竖直带状区域（下图）的面积就是 $(x_{i+1}-x_i)$ 乘以**当前 $y$ 方向上覆盖长度**。
...
根据上面的推导，我们现在需要快速求出当前扫描线 $y$ 轴上被至少一个矩形覆盖的长度。考虑使用线段树维护。

首先我们显然需要先离散化 $y$ 坐标，假设现在有四个纵坐标为 $1,3,7,10$，我们会把他们离散化成 $1,2,3,4$，但线段树维护的并不是这 $4$ 个点，而是他们之间的三个区间。即 $[1,3),[3,7),[7,10]$。因此，**$m$ 个不同的 $y$ 坐标，对应 $m-1$ 个线段树叶子，叶子 $i$ 表示 $[y_i, y_{i+1}]$**  。

根据线段树模板，应该初始化 `vector<Info> a(m)`。

我们定义每个基本区间中， $c_i$ 表示当前这个区间被多少个矩形覆盖。$c_i=0$ 表示没被覆盖
