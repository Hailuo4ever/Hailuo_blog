...
**线段树节点维护的是区间信息，懒标记维护的是尚未下发的区间操作。**

对于每一道线段树题，需要设计四个核心部分：

1. 节点保存什么信息；
2. 左右儿子的信息如何通过 `push_up()` 合并；
3. 完整区间执行修改后，如何通过 `apply()` 直接更新节点；
4. 多个懒标记如何进行函数复合。

如果无法仅根据节点已有信息和懒标记计算修改后的节点信息，就不能机械套用普通懒标记。此时应当重新建模，例如使用差分数组处理“区间加法 + 区间 $\gcd$”。
