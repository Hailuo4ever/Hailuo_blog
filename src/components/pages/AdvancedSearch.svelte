...
async function run(updateURL = false) {
	...
	try {
		...
		filters = available;
		if (!(query.q.trim() || query.category || query.tags.length)) {
			status = "idle";
			return;
		}
		...
		const page = Math.min(
			query.page,
			Math.max(1, Math.ceil(found.length / 10)),
		);
...
onMount(() => {
	const restore = () => {
		...
		lastURL = window.location.href;
		state = readSearch(new URLSearchParams(window.location.search));
		void run();
	};
	restore();
...
<section class="card-base p-6 md:p-9 mb-4 rounded-(--radius-large) text-75">
    ...
    <SearchTypeSwitch value={state.type} on:change={event => { state.type = event.detail; change(); }} />
    <form on:submit|preventDefault={() => { if (!composing) { state.page = 1; void run(true); } }}>
        ...
        <div class="flex gap-2">
            <input id="full-search-input" type="search" bind:value={state.q} placeholder={i18n(state.type === "problem" ? I18nKey.searchProblemHint : I18nKey.search)} on:input={event => { state.q = event.currentTarget.value; input(); }}
...
<section aria-busy={status === "loading"}>
    <h2 id="search-results-heading" tabindex="-1" class="text-75 px-2 mb-3 outline-none" aria-live="polite">
        {#if status === "ready"}{i18n(state.type === "problem" ? I18nKey.searchProblemCount : I18nKey.searchResultCount).replace("{count}", String(hits.length))}{/if}
    </h2>
    {#if state.match === "any"}
    ...
    {:else if !active}<p class="notice text-50">{i18n(state.type === "problem" ? I18nKey.searchProblemHint : I18nKey.searchTypeSomething)}</p>
